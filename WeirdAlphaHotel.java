...
public class WeirdAlphaHotel {
    ...
    public String remove(int room) {
        ...
        if (isAvailable(room) == false && this.usage > 0) {
            departingGuest = getGuestName(room);
     ...
     * with the same nameo, method returns the room of the first guest with that
    ...
    public int whichRoom(String guestName) {
        int roomNum = -1;
        //If Room is found return hash value == room number
        //if room not found then return nugative 1

        for(int i = 0; i <numberOfRooms; i ++){
            if(this.hotel[i] == null) {
                continue;
            } else if (this.hotel[i].equals(guestName)) {
                roomNum =i;
            }
        }
    ...
    public boolean isGuestHere(String guestname) {
        ...
        //First we would check if there is a vacancy if equal to true or false
        ...
        //If there is no vacancy then we would tell the user there is no avaliability to add more guests(False)
        //If there is vacancy then we would put the guestName through the hash statement
        // after receiving the int from hash method we would put the int through the isAvaliable method to return a true or false
        return hasGuest;
    }  // method isGuestHere
