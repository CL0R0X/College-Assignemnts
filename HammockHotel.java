...
public class HammockHotel {
    ...
    public boolean addUnique(String guestName) {
        //needs updating to adjust for redundant names

        //initialized the boolean value
        boolean ifPresent = false;
        boolean ifNone = false;
        //create a loop to go through the loop to find redundant names and reject them
        //adds guest if the loop finds no same name
        int assignedRoom = this.hashCode(guestName);
        // Explore the hammocks at this room.
        Node hammock = this.rooms[assignedRoom];
        // If the room is empty, the loop won't even run.
        while (!ifPresent && hammock != null ) {
            // Is the guest in this hammock? If yes, the !found above will break the loop.
            ifPresent = hammock.data.equals(guestName);
            // Move to the next hammock, in case we need to try; if null, the loop will end.
            hammock = hammock.next;
        }
        if (ifPresent == false) {
    ...
    public String[] toArray() {
        String[] tempArr = new String[this.usage];
        //have a for loop in which goes through every room/linked list and adds it to the temp array

        int count = 0;
        for(int i = 0; i < this.rooms.length; i++){
            Node roomNode = this.rooms[i];
            while(roomNode != null && count < this.usage) {
                tempArr[count] = roomNode.data;
    ...
    public void clear() {
        //this.usage =0;
        //need to set the data to null
        //must loop through all of the data to set null, while loop
        //decrease each usage during each loop
        for (int i = 0; i < this.rooms.length; i++) {
            rooms[i] = null;
        }
        this.usage = 0;
     ...
     */
    public int[] stats() {
       // for loop to go through each room and then count the nodes
       // add a while loop to go through the array nodes then count each time

       int[] numOfGuestPerNode = new int[this.rooms.length];
       int count = 0;
       for( int i = 0; i < this.rooms.length; i++){
           Node roomNode = this.rooms[i];
           while(roomNode !=null){
           ...
           numOfGuestPerNode[i] = count;
           count = 0;
       }

        return numOfGuestPerNode;
