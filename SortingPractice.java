...
public class SortingPractice {
    ...
    static int[] iterativeMergeSort(int[] array) {
        // create a temp array to hold all values
        int[] temp = new int[0];
        for (int size = 0; size < array.length / 2; size = size * 2) {
            for (int i = 1; i < array.length; i = i + 2 * size) {
                temp = merge(slice(array, size, size + size), slice(array, size + size, size * (2+size)));
