...
public class UsefulDresser {
    ...
    public int compareTo(UsefulDresser dresser1) {
        int sum = this.totalDrawers - dresser1.totalDrawers;

        return sum;

    }
    public int fullerThan(UsefulDresser dresser1){
        //The way to compare each dresser is through comparing the empty drawers
        //to find the empty drawers for each dresser is through subracting the total drawers from used drawer
        int sum = 1;

        int emptyDrawer2 = dresser1.totalDrawers - dresser1.usedDrawers;

        int emptyDrawer1 = this.totalDrawers - this.usedDrawers;
        //Divide the total and used drawer to get a ratio and compare if they are equivalent
        if(this.totalDrawers/this.usedDrawers == dresser1.totalDrawers/dresser1.usedDrawers){
            sum =0;
        } else if( emptyDrawer1 > emptyDrawer2 || emptyDrawer1 < emptyDrawer1) {
            //Subtract the empty drawers to get the sum
            sum = emptyDrawer2 - emptyDrawer1;
        }
        //if the sum is -1 then dresser2 is greater than a dresser1; 1 then dresser1 > dresser2; 0 then dresser1 = dresser2
